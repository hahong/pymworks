...
class MyInstall(Install):
    def run(self):
        cmd = 'cd %s/src && make install' % os.path.dirname(os.path.realpath(__file__))
        assert os.system(cmd) == 0, 'Failed to make _data.so'
        Install.run(self)
...
# kludge way of adding _data.so
package_data['pymworks'] += ['_data.*']
...
setuptools.setup(
    ...
    packages = packages,
    scripts = scripts,
    url = 'https://github.com/hahong/pymworks',
    author = 'Ha Hong',
    author_email = 'anon@anon.com',
    description = 'Python interface to .mwk files under *NIX systems',
